...
async def trigger_processing(
    ...
    task = process_audio_asset.delay(str(db_job.id), str(job_in.project_id) if job_in.project_id else None, "dummy_s3_key", job_in.job_type, job_in.options)
...
async def upload_audio_file(
    ...
    # Escribir siempre sobre input.wav para que Celery lo recoja (solución rápida single-user)
    local_path = os.path.join(local_dir, "input.wav")
    ...
    db_asset = AudioAsset(
        ...
        s3_key=f"uploads/{safe_id}_{file.filename}",
...
@router.post("/upload-stem", response_model=schemas.AudioAssetResponse)
async def upload_stem_file(
    db: Annotated[AsyncSession, Depends(get_db)],
    project_id: UUID | None = None,
    file: UploadFile = File(...)
):
    """Permite al frontend subir un stem individual explícito para procesamiento de IA."""
    local_dir = os.path.join("storage", "input")
    os.makedirs(local_dir, exist_ok=True)
    
    # Guardamos siempre como stem.wav
    local_path = os.path.join(local_dir, "stem.wav")
    with open(local_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    ...
    db_asset = AudioAsset(
        project_id=project_id,
        s3_key=f"uploads/stems/{safe_id}_{file.filename}",
        asset_type="stem_track",
        status="ready"
    )
    db.add(db_asset)
    await db.commit()
